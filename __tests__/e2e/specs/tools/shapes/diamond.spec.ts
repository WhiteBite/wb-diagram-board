...
test.describe('Diamond Tool', () => {
    ...
    test('should activate from toolbar', async ({ toolbar }) => {
        ...
        const activeTool = await toolbar.getActiveTool();
        expect(activeTool).toBe('diamond');
        expect(await toolbar.isToolActive('diamond')).toBe(true);
    });

    test('should create diamond on drag', async ({ canvas, toolbar }) => {
        await toolbar.selectTool('diamond');
    ...
    test('should create proportional diamond with Shift', async ({ canvas, toolbar }) => {
        await toolbar.selectTool('diamond');
        ...
        // With Shift, width and height should be equal (square diamond)
    ...
    test('should have minimum size', async ({ canvas, toolbar }) => {
        ...
        // Try to create very small diamond
        const id = await canvas.draw
            .from(100, 100)
            .to(102, 102)
            .execute();
    ...
    test('should create diamond with negative drag direction', async ({ canvas, toolbar }) => {
        await toolbar.selectTool('diamond');
    ...
    test('should create multiple diamonds in sequence', async ({ canvas, toolbar }) => {
        await toolbar.selectTool('diamond');
        ...
        // Tool should auto-switch to select, switch back to diamond
        await toolbar.selectTool('diamond');
        ...
        expect(element1!.type).toBe('diamond');
        expect(element2!.type).toBe('diamond');
